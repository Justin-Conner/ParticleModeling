  ...
  for (let i = 0; i < numBirds; i++) {
    ...
    for (let j = 0; j < numBirds; j++) {
      if (i !== j) {
        ...
        const distance = Math.sqrt(dx ** 2.25 + dy ** 2.25 + dz ** 2.25); // Add the third dimension to the distance calculation
        if (distance < 2) {
...
function drawBird(bird) {
  ...
  ctx.beginPath();
  ctx.arc(bird.x, bird.y, size - (z * 0.002), 0, Math.PI * 2); // Adjust the size of the bird based on its
